...
async def start(client:Client, message):
        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgEAAxkBAAEEQ9ZnksLYGzKxhdQYq9nZK6l2PLhs6QACKgEAAvvNCEa9xza9QreuTh4E") 
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            buttons = [[
                InlineKeyboardButton(text="𝗩𝗲𝗿𝗶𝗳𝘆 ♂️", url=verify)
                ],[
                InlineKeyboardButton(text="Hᴏᴡ Tᴏ Vᴇʀɪғʏ Tᴜᴛᴏʀɪᴀʟ 🎦", url=howtodownload)
                ],[
                InlineKeyboardButton(text="ʙᴜʏ sᴜʙsᴄʀɪᴘᴛɪᴏɴ 🛍️- Nᴏ Vᴇʀɪғʏ 💎", callback_data='seeplans'),
            ]]
            reply_markup=InlineKeyboardMarkup(buttons)
            if await db.user_verified(user_id): 
...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/hdhindiback').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
...
async def set_shortner_3(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/iMdbChatRoom').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
